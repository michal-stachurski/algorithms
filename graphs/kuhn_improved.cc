...
// Before finding augmenting path we'll find initial
// matching (arbitrary) using simple heuristic
...
int main() {
    ...
    // This is previously mentioned heuristic to
    // find arbitrary initial matching, forrandom graphs
    // it will generate sufficiently big matchings
    // Runtime: O(n+m)
    ...
    for (int v : side[0]) {
        if (used[v]) { // speed-up given by previous loop
            continue;
        }
